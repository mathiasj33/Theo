...
public class EpsilonNFA {
    ...
    public Set<State> computeReachableStates(Set<State> s, char c) { //computes e*ae*
        Set<State> result = new HashSet<>();
        for (State current : s) {
            for (Transition t : transitions) {
                if (t.getStart().equals(current) && t.getLabel() == c) {
                    result.addAll(epsilonClosure(t.getEnd()));
                }
            }
        }
        return result;
    ...
    public Set<State> epsilonClosure(State s) {
        ...
        while (!workList.isEmpty()) {
            State reached = workList.remove(0);
            result.add(reached);
            for (Transition t : transitions) {
                if (t.getStart().equals(reached) && t.getLabel() == Transition.EPSILON) {
                    if (!result.contains(t.getEnd())) workList.add(t.getEnd());
                }
            }
        }
        return result;
    ...
    private void checkValidEpsilonNFA() throws IllegalArgumentException {
        //No field is null
        if (states == null) {
            throw new IllegalArgumentException("EpsilonNFA constructor: states cannot be null.");
        }
        if (transitions == null) {
            throw new IllegalArgumentException("EpsilonNFA constructor: transitions cannot be null.");
        }
        if (alphabet == null) {
            throw new IllegalArgumentException("EpsilonNFA constructor: alphabet cannot be null.");
        }
        if (startState == null) {
            throw new IllegalArgumentException("EpsilonNFA constructor: startState cannot be null.");
        }
        if (finalStates == null) {
            throw new IllegalArgumentException("EpsilonNFA constructor: finalStates cannot be null.");
        }
        ...
        for (Transition t : transitions) {
            ...
            if (!alphabet.contains(t.getLabel()) && t.getLabel() != Transition.EPSILON) {
